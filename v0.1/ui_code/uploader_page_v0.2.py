...
def separate_and_upload_to_postgres(folder_path, file_type, dropdown_data, user_id,total_files):
    key_order = ['mill_name', 'machine_number_id', 'roll_id', 'rotation', 'cam', 'Date', 'Hours', 'mins', 'seconds', 'angle', 'time_stamp']
    files_uploaded=0
    process_bar =st.progress(0)
    placeholder=st.empty()
    progress=0
    destination_directory = '/home/joel/Desktop/processed'
...
def process_fda_files(selected_folder_path, dropdown_data):
    ...
    for root, _, files in os.walk(selected_folder_path):
        for file_name in files:
            ...
            process_bar.progress(progress)
    # st.write("Files_Found at P FDA ",files_processed)
    process_bar.progress(1.0)
    process_bar.empty()
    placeholder.empty()    
    ...
    if selected_folder_path: 
        ...
        if st.button("Upload"):
            if filetype == "MDD":
                ...
                mdd_tot=mdd_copy(selected_folder_path)
                
                append_json_to_files(destination_directory, dropdown_data, file_type)  
                files_uploaded,total_files= separate_and_upload_to_postgres(selected_folder_path, file_type, dropdown_data, st.session_state.get('user_id'),mdd_tot)   
                
                if files_uploaded<mdd_tot:
                    
                    st.info(f'{mdd_tot-(files_uploaded-1)} Duplicate files Found . These are skipped') 
                    files_uploaded=files_uploaded-1        
                if files_uploaded>0:           
                    Trigger()
                ...
                dropdown_data = load_dropdown_data()
                p_files,fdd_tot=process_fda_files(selected_folder_path,dropdown_data)
                append_json_to_files(destination_directory, dropdown_data, file_type)  
               
                files_uploaded,total_files= separate_and_upload_to_postgres(selected_folder_path, file_type, dropdown_data, st.session_state.get('user_id'),fdd_tot)   
                
                

                if files_uploaded<fdd_tot:
                    
                    st.info(f'{fdd_tot-(files_uploaded-1)} Duplicate files Found . These are skipped')
                    files_uploaded=files_uploaded-1  
...
def execute_curl_command_upload():
    try:
        ...
        subprocess.run(["/usr/bin/curl", "-X", "POST", "http://localhost:8080/api/v1/dags/scp_send_2/dagRuns", "-H", "Content-Type: application/json", "-H", "Accept: application/json", "-u", "airflow:airflow", "-d", '{"note":"streamlit run"}'], check=True)
        return True
